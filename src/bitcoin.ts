...
export class RESTClient extends RPC {
   ...
   */
  constructor({ url = 'http://localhost', port, timeout = 30000, ...options }: RESTIniOptions = {}) {
    if (port) {
      super({ ...options, baseUrl: `${url}:${port}`, timeout, json: true });
    } else {
      super({ ...options, baseUrl: url, timeout, json: true });
    }
  }
...
export type Descriptor = string | { desc: string; range: number | [number, number] };
...
export type TransactionOutput = { [address: string]: string | number } | { data: string };
...
export type BumpFeeParams = {
  txid: string;
  options?: EstimateMode & { replaceable?: boolean } & ({ confTarget?: number } | { totalFee?: number });
};
...
export type WalletCreateFundedPsbtParams = BaseCreateTransaction & BaseFundOptions & { bip32derivs?: boolean };
...
export class RPCClient extends RESTClient {
   ...
   */
  getrawchangeaddress(options: { address_type?: AddressType }, wallet?: string) {
   ...
   * @description Sign inputs for raw transaction
   */
  signrawtransactionwithwallet(options: SignRawTransactionWithWalletParams, wallet?: string) {
    return this.rpc('signrawtransactionwithwallet', options, wallet || this.wallet);
  }
   ...
   */
  walletcreatefundedpsbt(options: WalletCreateFundedPsbtParams, wallet?: string) {
   ...
   */
  walletpassphrasechange(options: WalletPassphraseChangeParams, wallet?: string) {
